...
public class CtrlProduit implements Initializable{
	...
	private void creerModele(ActionEvent e){
			...
			boolean exit = false;
			for(Produit p1: DAOFactory.getDAOFactory(Persistance.LISTEMEMOIRE).getProduitDAO().findAll())
			...
			DAOFactory.getDAOFactory(Persistance.LISTEMEMOIRE).getProduitDAO().create(p);
...
//			if((tblProduits.getSelectionModel().getSelectedItem()).getId()==3){
//				Image image = new Image(getClass().getResourceAsStream("images/p3.png"),40,40,false,false);
...
//			}
			...
			DAOFactory.getDAOFactory(Persistance.LISTEMEMOIRE).getProduitDAO().update(p);
	...
	private void supprimerAction(ActionEvent e){
		...
		for(Ligne_facture lf: DAOFactory.getDAOFactory(Persistance.LISTEMEMOIRE).getlignefactureDAO().findAll())
		...
		DAOFactory.getDAOFactory(Persistance.LISTEMEMOIRE).getProduitDAO().delete(t);
	...
	public void initialize(URL location, ResourceBundle resources) {
		this.cbxTva.setItems(FXCollections.observableArrayList(DAOFactory.getDAOFactory(Persistance.LISTEMEMOIRE).getTVADAO().findAll()));
		...
		this.tblProduits.getItems().addAll(DAOFactory.getDAOFactory(Persistance.LISTEMEMOIRE).getProduitDAO().findAll());
		btnsupprimer.setDisable(true);
		btnajoute.setDisable(false);
		btnmodifier.setDisable(true);
