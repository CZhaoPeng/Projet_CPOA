...
public class CtrlLignefactureSQL implements Initializable{
	@FXML
	...
	private void creerModele(ActionEvent e){
			...
			DAOFactory.getDAOFactory(Persistance.MYSQL).getlignefactureDAO().create(lf);
			...
			lf.setQuantite(Integer.parseInt(tf_quantite.getText()));
			lf.setIdproduit(cbxProduit.getSelectionModel().getSelectedItem());
			DAOFactory.getDAOFactory(Persistance.MYSQL).getlignefactureDAO().update(lf);
	...
	private void selectionAction(){
		lf = tblLignefacture.getSelectionModel().getSelectedItem();
		...
		for(Facture f: DAOFactory.getDAOFactory(Persistance.MYSQL).getFactureDAO().findAll())
			if(f.getId()==lf.getId()){
	...
	private void supprimerAction(ActionEvent e){
		...
		DAOFactory.getDAOFactory(Persistance.MYSQL).getlignefactureDAO().delete(t);
	...
	public void initialize(URL location, ResourceBundle resources) {
		this.cbxFacture.setItems(FXCollections.observableArrayList(DAOFactory.getDAOFactory(Persistance.MYSQL).getFactureDAO().findAll()));
		this.cbxProduit.setItems(FXCollections.observableArrayList(DAOFactory.getDAOFactory(Persistance.MYSQL).getProduitDAO().findAll()));
		...
		this.tblLignefacture.getItems().addAll(DAOFactory.getDAOFactory(Persistance.MYSQL).getlignefactureDAO().findAll());
		btnsupprimer.setDisable(true);
		btnajoute.setDisable(false);
		btnmodifier.setDisable(true);
